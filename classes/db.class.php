...
var $databaseLink;      // Database Connection
    var $lastId;
...
$this->database = 'php_1_01_data_base';
$this->username = 'k_oleh';
$this->password = 'korn123';
...
// Adds a record to the database based on the array key names
public function insert_geo($lat, $lon){

$queryStr_loc_in = 'INSERT INTO Locations (poi_location, poi_type, poi_type_style, poi_center, poi_radius, IdUser) VALUES (';
$queryStr_loc_in .= "GeomFromText('POINT(".$lat." ".$lon.")'), 'point', 0, PointFromText('POINT(".$lat." ".$lon.")'), 0, 1";
$queryStr_loc_in .= ')';

$this->executeSQL($queryStr_loc_in);
...
return $this->executeSQL($query);
    ...
    public function select_name($from, $where='', $orderBy='', $limit='', $like=false, $operand='AND',$cols='*'){
// Catch Exceptions
        if(trim($from) == ''){
            return false;
        }
...
$result = $this->select($from, $where, '', '', false, 'AND','count(*)', array());
return $result["count(*)"];
}
